...
El directorio `./iot-admin-panel` aloja el código fuente de la **WebApp** de administración desplegada en **Microsoft Azure**. Esta aplicación actúa como el panel de control centralizado para la gestión de los dispositivos de intercomunicación (ESP32-EYE). El flujo funcional es el siguiente:
...
3. **Provisión de Dispositivos**: El sistema permite el alta de nuevos dispositivos introduciendo su dirección MAC, ubicación física y un alias identificativo.
...
5. **Persistencia de Datos**: Toda la información relacional (usuarios, inventario de dispositivos, logs y credenciales) se almacena en una instancia de Azure Database for MySQL.

Para obtener los datos de los dispositivos creados se puede usar este comando:

```bash
$ curl -v -X GET https://iot-admin-panel-a7dub3cmf6f3a4hc.spaincentral-01.azurewebsites.net/api/edge/sync-devices -H "x-edge-api-key: clave_segura_comunicacion_edge_cloud_DII_2025_26"
```

Y para cambiar el campo status de un dispositivo:

```bash
curl -v -X POST https://iot-admin-panel-a7dub3cmf6f3a4hc.spaincentral-01.azurewebsites.net/api/edge/device-status \
