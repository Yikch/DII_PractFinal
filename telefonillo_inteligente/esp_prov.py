...
async def mqtt_port(tp, sec, url):
    try:
        message = prov.custom_data_request(sec, url)
        response = await tp.send_data('mqtt-port', message)
        return (prov.custom_data_response(sec, response) == 0)
    except RuntimeError as e:
        on_except(e)
        return None

async def mqtt_topic(tp, sec, url):
    try:
        message = prov.custom_data_request(sec, url)
        response = await tp.send_data('mqtt-topic', message)
        return (prov.custom_data_response(sec, response) == 0)
    except RuntimeError as e:
        on_except(e)
        return None
...
async def main():
    ...
    parser.add_argument('--mqtt_url', dest='mqtt_url', type=str, default='',
                        help=desc_format(
                            'This is a parameter to provide the url for '
                            'thingsboard server'))

    parser.add_argument('--mqtt_port', dest='mqtt_port', type=str, default='',
                        help=desc_format(
                            'This is a parameter to provide the port for '
                            'thingsboard server'))

    parser.add_argument('--mqtt_topic', dest='mqtt_topic', type=str, default='',
                        help=desc_format(
                            'This is a parameter to provide the port for '
                            'thingsboard server'))
    ...
    try:
        ...
        if args.mqtt_url != '':
            print('\n==== Sending MQTT URL to Target ====')
            if not await mqtt_url(obj_transport, obj_security, args.mqtt_url):
                raise RuntimeError('Error in Thingsboard URL')
            print('==== MQTT URL sent successfully ====')

        if args.mqtt_port != '':
            print('\n==== Sending MQTT PORT to Target ====')
            if not await mqtt_port(obj_transport, obj_security, args.mqtt_port):
                raise RuntimeError('Error in Thingsboard CNF')
            print('==== MQTT PORT sent successfully ====')

        if args.mqtt_topic != '':
            print('\n==== Sending MQTT TOPIC to Target ====')
            if not await mqtt_topic(obj_transport, obj_security, args.mqtt_topic):
                raise RuntimeError('Error in Thingsboard CNF')
            print('==== MQTT TOPIC sent successfully ====')
